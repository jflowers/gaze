...
- fix-assertion-detection-confidence: Fixed `buildExternalQualityReports` hardcoding `AssertionDetectionConfidence` to 0 for external analyzers (#251). Added `computeDetectionConfidenceFromMappings` to `internal/adapter/contract.go` — mirrors `quality.computeDetectionConfidence` semantics (recognized non-empty `AssertionType` / total, integer truncation). Added `detectionConfidence map[string]int` field and `DetectionConfidence(pkg, function string) int` method to `ExternalContractCoverageProvider`. `Build` now computes per-target-function confidence after `buildContractLookup`. In `buildExternalQualityReports` (`cmd/gaze/main.go`), comma-ok type assertion populates `AssertionDetectionConfidence` on each `QualityReport` and summary (arithmetic mean matching Go-native path). Extended fake analyzer `test_mapping` from 1 to 3 mappings for integration testing. 13 new tests (9 table-driven unit, 3 method, 1 integration). No protocol changes. Closes #251.
