...
3. **Release** (`.github/workflows/release.yml`): Triggered via `workflow_dispatch` with a `tag` input (e.g., `v0.15.0`). Delegates to `complytime/org-infra` reusable workflows for preflight validation (`reusable_release_preflight.yml`) and GoReleaser execution (`reusable_release_goreleaser.yml`). Preflight validates tag format, uniqueness (with smart re-run detection), semver ordering (Python `semver` library), CI checks on HEAD (`Unit + Integration Tests (Go 1.24)`, `Unit + Integration Tests (Go 1.25)`, `MegaLinter`), and unreleased commits, then creates an annotated tag via GitHub API. GoReleaser builds cross-platform binaries (darwin/linux x amd64/arm64), creates GitHub Releases, and uploads the Homebrew cask as a release asset. The reusable GoReleaser workflow produces cosign signatures and SBOMs for supply chain verification. Optional skip inputs (`skip_semver_check`, `skip_ci_checks`, `skip_unreleased_check`) allow bypassing individual checks for debugging. macOS signing and Homebrew tap publishing remain inline. When macOS signing secrets are unavailable (e.g., in forks), the unsigned cask is pushed directly to the Homebrew tap. To release: use the GitHub Actions "Run workflow" button or `gh workflow run release.yml -f tag=v1.2.3`.
...
- fix-assertion-detection-confidence: Fixed `AssertionDetectionConfidence` being hardcoded to 0 for external analyzers (#251). Added unexported `classificationConfidence(mappings []protocol.AssertionMappingData) int` helper to `internal/adapter/quality.go` — a proxy for assertion-detection confidence computed as recognized non-empty `AssertionType` / total emitted mapping rows (integer truncation). It is NOT identical to the Go-native `quality.computeDetectionConfidence` denominator, which counts all detected assertion sites (the external protocol does not expose that count). `BuildQualityFromMappings` now populates `AssertionDetectionConfidence` per test function, and `buildQualitySummary` aggregates it as an arithmetic mean. Extended fake analyzer `test_mapping` from 1 to 3 mappings for integration testing. No protocol changes. Closes #251.
- quality-external-analyzer: Wired `--analyzer` and `--language` flags to `gaze quality` for external test_mapping support (Issue #229, lifting D12 deferral). Added `BuildQualityFromMappings` in `internal/adapter/quality.go` — converts `[]protocol.AssertionMappingData` + `[]taxonomy.AnalysisResult` into `[]taxonomy.QualityReport` + `*taxonomy.PackageSummary`, reusing `quality.ComputeContractCoverage` for metric computation. Added `FetchTestMappings` standalone function for direct `test_mapping` protocol calls. Added `computeOverSpecification` for incidental-effect assertion counting. Added `Session.Client()` accessor to `internal/adapter/session.go`. Exposed `SideEffects *ExternalSideEffectAnalyzer` on `Providers` struct. In `cmd/gaze/main.go`: replaced `--analyzer` rejection block with `runQualityWithExternalAnalyzer` following `runCrapWithExternalAnalyzer` pattern; added `handleQualityNoTestMapping` and `handleQualityTestMappingError` for graceful degradation; unhid `--analyzer`/`--language` flags in `newQualityCmd`. Flag validation: `--target` and `--ai-mapper` rejected with `--analyzer` (Go-specific SSA/AST features). 14 new tests across `internal/adapter/` and `cmd/gaze/`. Closes #229.
...
- external-analyzer-protocol: Added JSON-RPC 2.0 protocol for external language analyzer communication. New `internal/protocol/` package with `Client` (subprocess management, request/response sequencing, context-based timeouts), 8 method constants, and full request/response/error types. New `internal/adapter/` package with four provider adapters (`ExternalComplexityProvider`, `ExternalLineCoverageProvider`, `ExternalSideEffectAnalyzer`, `ExternalContractCoverageProvider`) implementing Phase 1 interfaces, plus `Session` (lifecycle management) and `Discover` (three-tier analyzer discovery: CLI flag → `.gaze.yaml` config → PATH convention). Added `--analyzer` and `--language` flags to `crap`, `quality`, and `report` commands. Added `AnalyzersConfig` to `internal/config/config.go`. Fake analyzer binary at `internal/protocol/testdata/fake_analyzer/` for integration testing. Closes #95.
