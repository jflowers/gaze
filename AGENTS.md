...
- fix-assertion-detection-confidence: Fixed `buildExternalQualityReports` hardcoding `AssertionDetectionConfidence` to 0 for external analyzers (#251). Added `computeMappingClassificationConfidence` to `internal/adapter/contract.go` — a proxy for assertion-detection confidence computed as recognized non-empty `AssertionType` / total emitted mapping rows (integer truncation). It is NOT identical to the Go-native `quality.computeDetectionConfidence` denominator, which counts all detected assertion sites (the external protocol does not expose that count). Added `classificationConfidence map[string]int` field and `MappingClassificationConfidence(pkg, function string) int` method to `ExternalContractCoverageProvider`; `Build` resets the map to nil up front so degraded paths never return stale values. In `buildExternalQualityReports` (`cmd/gaze/main.go`), comma-ok type assertion populates `AssertionDetectionConfidence` on each `QualityReport` and summary (arithmetic mean matching Go-native path). Extended fake analyzer `test_mapping` from 1 to 3 mappings for integration testing. 13 new tests (9 table-driven unit, 3 method, 1 integration) plus a report-path regression test. No protocol changes. Closes #251.
