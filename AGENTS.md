...
- fix-incidental-type-guard: Fixed bug where incidental naming/godoc penalties (`log`/`Log`/`debug`/`trace`/`print` prefixes, `logs`/`prints`/`traces`/`debugs` keywords) applied to all side effect types including P0 `ReturnValue`, causing misclassification of return values on functions like `LogAndCompute` as incidental. Converted `incidentalPrefixes` in `internal/classify/naming.go` and `incidentalKeywords` in `internal/classify/godoc.go` from `[]string` to typed slices with `appliesTo []taxonomy.SideEffectType`, scoped to I/O effect types only (`LogWrite`, `StdoutWrite`, `StderrWrite`). Added shared `ioEffectTypes` variable. Updated 3 existing tests, added `TestNamingSignal_IncidentalTypeGuard` (10 cases), `TestAnalyzeGodocSignal_IncidentalTypeGuard` (7 cases), and 2 integration tests through `ComputeScore`. Closes #105.
