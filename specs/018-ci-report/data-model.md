...
```go
// ReportSummary holds pre-extracted threshold-relevant metric values from the
// analysis pipeline. It is populated during Run() so that EvaluateThresholds
// can compare typed int values without unmarshalling the raw JSON fields.
// ReportSummary is internal to the pipeline; it is NOT serialised to the
// --format=json output.
